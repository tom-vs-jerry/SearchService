...
using WebSocket.SocketEngine.Configuration;
...
namespace WebSocket.SocketEngine
{
    ...
    public partial class DefaultBootstrap : IBootstrap, ILoggerProvider, IDisposable
    {
        ...
        {
            var bootstrapIpcPort = string.Format("WebSocket.Bootstrap[{0}]", Math.Abs(AppDomain.CurrentDomain.BaseDirectory.TrimEnd(System.IO.Path.DirectorySeparatorChar).GetHashCode()));
