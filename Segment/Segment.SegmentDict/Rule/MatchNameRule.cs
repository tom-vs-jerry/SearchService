...
namespace Segment.SegmentDict.Rule
{
    ...
    class ChsNameTraffic
    {
        ...
        public List<ChsNameWordTraffic> Words = new List<ChsNameWordTraffic>();
    ...
    {
        ChsNameTraffic m_Traffic;
        Hashtable m_Table = new Hashtable();

        public void Clear()
        {
            m_Table = new Hashtable();
            m_Traffic = new ChsNameTraffic();
        }

        public void Save(String fileName)
        {
            //var op = new KTDictSeg.CPOS();
            //KTDictSeg.MatchName mn = new KTDictSeg.MatchName(op);
            //mn.LoadNameTraffic(@"D:\工具\v1.4.02\Release\Data\Name.dct");
            //KTDictSeg.CChsNameTraffic cc = mn.m_ChsNameTraffic;
            
            //m_Traffic = new ChsNameTraffic();
            //m_Traffic.AfterCount = cc.m_Traffic.AfterCount;
            //m_Traffic.AfterWordCount = cc.m_Traffic.AfterWordCount;
            //m_Traffic.BeforeCount =cc.m_Traffic.BeforeCount;
            //m_Traffic.BeforeWordCount = cc.m_Traffic.BeforeWordCount;
            ////m_Traffic.Words.Add;            

            //foreach (KTDictSeg.T_ChsNameWordTraffic wordTraffic in cc.m_Traffic.Words)
            //{
            //    ChsNameWordTraffic cns = new ChsNameWordTraffic();
            //    cns.After = wordTraffic.After;
            //    cns.Before = wordTraffic.Before;
            //    cns.Word = wordTraffic.Word;
            //    m_Traffic.Words.Add(cns);
            //}
        ...
        public void Load(String fileName)
        {
            try
            {
                List<ChsNameWordTraffic> newWords = new List<ChsNameWordTraffic>();
                ...
                //整理姓名前缀后缀表
                foreach (ChsNameWordTraffic wordTraffic in m_Traffic.Words)
                {
        ...
        public void AddBefore(String word)
        {
            ...
            ChsNameWordTraffic wordTraffic = (ChsNameWordTraffic)m_Table[word];
            if (wordTraffic == null)
            {
                wordTraffic = new ChsNameWordTraffic();
        ...
        {
            ChsNameWordTraffic cw1 = GetWordTraffic(fst);
            ChsNameWordTraffic cw2 = GetWordTraffic(sec);
        ...
        public ChsNameWordTraffic GetWordTraffic(String word)
        {
            return (ChsNameWordTraffic)m_Table[word]; ;
        }

        public bool MaybeNameByAfter(String afterWord)
        {
            ChsNameWordTraffic wordTraffic = GetWordTraffic(afterWord);
            if (wordTraffic == null)
            {
                return false;
            }
        ...
        {
            ChsNameWordTraffic wordTraffic = GetWordTraffic(beforeWord);
            if (wordTraffic == null)
            {
                return false;
            }
    ...
    {
        public delegate void TrafficUnknownWordFunc(String word, EnumPOS Pos);

        CChsNameTraffic chsNameTraffic;
        PosBinRule posBinRule;
        CPOS pos;
        TrafficUnknownWordFunc trafficUnknownWordHandle;
        ...
        public TrafficUnknownWordFunc TrafficUnknownWordHandle
        {
            get
            {
                return trafficUnknownWordHandle;
            }

            set
            {
                trafficUnknownWordHandle = value;
            }
        }

        public MatchName(CPOS Pos)
        {
            posBinRule = new PosBinRule(pos);
            pos = Pos;
            chsNameTraffic = new CChsNameTraffic();
        ...
        private void Traffic(String beforeWord, String afterWord)
        {
            if (beforeWord != null)
            {
                if (beforeWord.Trim() != "")
                {
                    chsNameTraffic.AddBefore(beforeWord);
                }
            }
            ...
            if (afterWord != null)
            {
                if (afterWord.Trim() != "")
                {
                    chsNameTraffic.AddAfter(afterWord);
                }
            }
        }
            ...
            */
            ...
            if (name.Length - familyName.Length == 1)
            {
                ...
                if (index < preWords.Count - 2)
                {
                    ...
                    if (nnext.Length >= 2)
                    {
                        if (!chsNameTraffic.MaybeNameByAfter(nnext))
                        {
                            String after = nnext.Substring(1, nnext.Length - 1);
                            nnext = nnext[0].ToString();

                            if (chsNameTraffic.CompareTwoWords(nnext, after))
                            {
                                name += nnext;
                                retWords.Add(name);
                                retWords.Add(after);
                        ...
                        nnext[0] >= 0x4e00 && nnext[0] <= 0x9fa5)
                    {
                        ...
                        if (index + 3 < preWords.Count)
                        {
                            merge = chsNameTraffic.CompareTwoWords(preWords[index + 2], preWords[index + 3]);
                        }

                        if (!merge)
                        {
                            merge = !posBinRule.MatchNameInHead(nnext);
                        }
            ...
            else if (name.Length - familyName.Length > 2)
            {
                ...
                if (nnext.Length > 1)
                {
                    if (posBinRule.MatchNameInHead(nnext.Substring(1, nnext.Length - 1)))
                    {
                        //统计
                        if (autoStudy)
                        {
        ...
        private int MatchFamilyNameInTailByTraffic(List<String> preWords, int index, List<String> retWords)
        {
            ...
            if (retWords.Count > 0)
            {
                ...
                if (retWords.Count > 1)
                {
                    ...
                    if (!chsNameTraffic.MaybeNameByBefore(newWord))
                    {
                        newWord = null;
                    }
            ...
            if (name.Length - familyName.Length == 1)
            {
                ...
                if (index < preWords.Count - 1)
                {
                    ...
                    if (nnext.Length <= 2)
                    {
                        ...
                        if (index + 2 < preWords.Count)
                        {
                            merge = chsNameTraffic.CompareTwoWords(preWords[index + 1], preWords[index + 2]);
                        }
        ...
        private int MatchFamilyNameInTail(List<String> preWords, int index, List<String> retWords)
        {
            ...
            if (retWords.Count > 0)
            {
                ...
                if (retWords.Count > 1)
                {
                    ...
                    {
                        if (!posBinRule.MatchNameInTail(newWord))
                        {
                            newWord = null;
                        }
                    }
            ...
            if (name.Length - familyName.Length == 1)
            {
                ...
                if (index < preWords.Count - 1)
                {
                    ...
                    if (nnext.Length <= 2)
                    {
                        if (!posBinRule.MatchNameInHead(nnext))
                        {
                            name = name + (String)preWords[index + 1];
                            retWords.Add(name);
                            return index + 2;
        ...
        private int SimpleMatch(List<String> preWords, int index, List<String> retWords)
        {
            ...
            if (prefix != null)
            {
                if (chsNameTraffic.MaybeNameByBefore(prefix))
                {
                    isNameByPrefix = true;
                }
            }
            ...
            {
                if (chsNameTraffic.MaybeNameByAfter(preWords[postfixPosition]))
                {
                    //如果姓后面直接跟后缀，直接返回姓
                ...
                else
                {
                    if (preWords[postfixPosition].Length == 1)
                    {
                        ...
                        else if (chsNameTraffic.MaybeNameByAfter(preWords[postfixPosition]))
                        {
                            //是后缀
                            isNameByPostfix = true;
                        }
            ...
            if (isNameByPostfix || isNameByPrefix)
            {
                ...
                if (AutoStudy)
                {
                    ...
                    {
                        TrafficUnknownWordHandle(name, EnumPOS.POS_A_NR);
                    }
                }
