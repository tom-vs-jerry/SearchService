...
namespace Segment
{
    ...
    //@"[０-９\d]+|[^ａ-ｚＡ-Ｚa-zA-Z0-9０-９\u4e00-\u9fa5]|[ａ-ｚＡ-Ｚa-zA-Z]+|[\u4e00-\u9fa5]+";
        ...
        private Dict.WordDictionary wordDictionary = null;

        public Dict.WordDictionary WordDictionary
        {
        ...
        private bool MergeEnglishSpecialWord(string orginalText, SuperLinkedList<WordInfo> wordInfoList, ref SuperLinkedListNode<WordInfo> current)
        {
            ...
            if (last >= 0)
            {
                ...
                WordAttribute wa = WordDictionary.GetWordAttr(newWord);
            ...
            while (cur != null)
            {
                ...
                switch (cur.Value.WordType)
                {
                    case WordType.SimplifiedChinese:
                        ...
                        Framework.AppendList<Dict.PositionLength> pls = WordDictionary.GetAllMatchs(inputText, _Options.ChineseNameIdentify);
                        Match.ChsFullTextMatch chsMatch = new Match.ChsFullTextMatch(WordDictionary);
        ...
        public ICollection<WordInfo> DoSegment(string text, Match.MatchOptions options, Match.MatchParameter parameters)
        {
            ...
            try
            {
                Dict.DictionaryLoader.Lock.Enter(Framework.Lock.Mode.Share);
                _Options = options;
        ...
        {
            WordDictionary = new Dict.WordDictionary();
            string dir = Setting.SegmentSettings.Config.GetDictionaryPath();
            WordDictionary.Load(dir + "Dict.Dct");

            _ChsName = new Dict.ChsName();
            _ChsName.LoadChsName(Setting.SegmentSettings.Config.GetDictionaryPath());


            WordDictionary.ChineseName = _ChsName;

            _StopWord = new Dict.StopWord();
            _StopWord.LoadStopwordsDict(dir + "Stopword.txt");

            _Synonym = new Dict.Synonym();

            if (Setting.SegmentSettings.Config.MatchOptions.SynonymOutput)
            {
                _Synonym.Load(dir);
            }

            _DictLoader = new Dict.DictionaryLoader(Setting.SegmentSettings.Config.GetDictionaryPath());
        }
        ...
        public void SaveDic()
        {
            ...
            string dir = Setting.SegmentSettings.Config.GetDictionaryPath();
            WordDictionary.Save(dir + "Dict.Dct");
        }

        public void SaveDic(string path)
        {
            //FormInputDictVersion frmInputDictVersion = new FormInputDictVersion();
            //frmInputDictVersion.Version = _Version;
            //string dir = Setting.SegmentSettings.Config.GetDictionaryPath();
            WordDictionary.Save(path + "Dict.Dct");
        }
        public void Init(string fileName)
        {
            lock (_LockObj)
            {
                ...
                if (fileName == null)
                {
                    Setting.SettingLoader loader = new Setting.SettingLoader();
                }
                else
                {
                    Setting.SettingLoader loader = new Setting.SettingLoader(fileName);
                }
                ...
                _Wildcard = new Dict.Wildcard(Setting.SegmentSettings.Config.MatchOptions,
                    Setting.SegmentSettings.Config.Parameters);

                string dir = Setting.SegmentSettings.Config.GetDictionaryPath();

                if (Setting.SegmentSettings.Config.MatchOptions.WildcardOutput)
                {
                    _Wildcard.Load(dir);
                }
