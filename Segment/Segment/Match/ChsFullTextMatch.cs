...
namespace Segment.Match
{
    public class ChsFullTextMatch: IChsFullTextMatch
    {
        ...
        Framework.AppendList<Node> _LeafNodeList = new Framework.AppendList<Node>();
        Dict.PositionLength[] _PositionLengthArr;
        int _InputStringLength;
        int _PositionLengthArrCount;

        List<Dict.PositionLength[]> _AllCombinations = new List<Dict.PositionLength[]>();
        ...
        {
            LinkedList<Dict.PositionLength> result = new LinkedList<Dict.PositionLength>();
        ...
        /// <returns></returns>
        private Node[] GetLeafNodeArray(Dict.PositionLength[] positionLenArr, int orginalTextLength, int count)
        {
            ...
            for (int i = 1; i < count; i++)
            {
                if (positionLenArr[i].Position >= lastRightBoundary)
                {
                    ...
                    int c = i - lastIndex;
                    Dict.PositionLength[] arr = new Dict.PositionLength[c];
        ...
        /// <returns></returns>
        private Node[] GetLeafNodeArrayCore(Dict.PositionLength[] positionLenArr, int orginalTextLength, int count)
        {
            _LeafNodeList.Clear();
        ...
        public SuperLinkedList<WordInfo> Match(Dict.PositionLength[] positionLenArr, string orginalText, int count)
        {
            if (_Options == null)
            {
            ...
            foreach (Node node in leafNodeArray)
            {
                ...
                Dict.PositionLength[] comb = new Dict.PositionLength[node.AboveCount];
            ...
            {
                Dict.PositionLength[] comb = new Dict.PositionLength[orginalText.Length];

                for (int i = 0; i < comb.Length; i++)
                {
                    Dict.PositionLength pl = new Dict.PositionLength(i, 1, new WordAttribute(orginalText[i].ToString(), POS.POS_UNK, 0));
                    pl.Level = 3;
                    comb[i] = pl;
                }
